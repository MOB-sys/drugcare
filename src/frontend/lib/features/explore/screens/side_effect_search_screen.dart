...
class SideEffectSearchScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SideEffectSearchScreen> createState() =>
      _SideEffectSearchScreenState();
}

/// [SideEffectSearchScreen]의 상태 관리 클래스.
class _SideEffectSearchScreenState
    extends ConsumerState<SideEffectSearchScreen> {
  /// 검색 입력 텍스트 컨트롤러.
  ...
  /// 검색 결과.
  PaginatedResult<SideEffectSearchItem>? _result;
  ...
  Future<void> _performSearch() async {
    ...
    try {
      ...
      final result = await service.searchBySideEffect(
        _query,
        page: _currentPage,
      );
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('부작용으로 약물 찾기')),
      body: Column(
        children: [
          _buildSearchField(),
  ...
  Widget _buildBody() {
    if (_isLoading) {
      return const LoadingWidget(message: '부작용별 약물을 검색 중...');
    }
