...
class ResultScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final resultAsync = ref.watch(interactionResultProvider(selectedItems));
    ...
    return Scaffold(
      ...
      body: resultAsync.when(
        loading: () => const LoadingWidget(message: '상호작용을 분석 중입니다...'),
        error: (error, _) => AppErrorWidget(
          message: '상호작용 확인 중 오류가 발생했습니다.\n다시 시도해 주세요.',
          onRetry: () => ref.invalidate(interactionResultProvider(selectedItems)),
        ),
        data: (response) {
