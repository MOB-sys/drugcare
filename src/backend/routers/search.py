...
async def suggest_all(
    ...
    약물, 영양제, 식품, 한약재에서 각 타입당 최대 limit/4개씩 검색하여
    관련성 높은 순으로 합쳐서 반환한다.
    """
    per_type = max(3, limit // 4)

    drugs = await drug_service.suggest_drugs(db, redis, q, per_type)
    supplements = await supplement_service.suggest_supplements(db, redis, q, per_type)
    foods = await food_service.suggest_foods(db, redis, q, per_type)
    herbals = await herbal_medicine_service.suggest_herbal_medicines(db, redis, q, per_type)
